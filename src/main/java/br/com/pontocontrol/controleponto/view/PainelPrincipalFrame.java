...
public class PainelPrincipalFrame extends javax.swing.JFrame {
   ...
   public void atualizarValores() {
      ...
      cmpTotalMes.setText(String.format("%s - %s", folhaPontoMensalViewState.getTotalMensal(), folhaPontoMensalViewState.getTotalEsperadoMensal()));
      cmpTotalMes.setToolTipText(String.format("Variação: %s", folhaPontoMensalViewState.getVariacaoMensal()));
      labelPeríodo.setText(folhaPontoMensalViewState.getIntervaloAtualAsString());
      folhaPontoMensalViewState.getListaRegistros().forEach((entry) -> {
   ...
   private void doEditar() {
      if (tabelaRegistros.getSelectedColumnCount() == 1 || tabelaRegistros.getSelectedRowCount() == 1) {
         ...
         RegistroDiarioPonto registroDiario = folhaPontoMensalViewState.getRegistroDiario(dia);
         Calendar data = Calendar.getInstance();
         data.setTime(folhaPontoMensalViewState.getIntervaloAtual());
         data.set(Calendar.DAY_OF_MONTH, dia);
         if (registroDiario != null) {
            final EditarRegistroFrame janelaEditar = new EditarRegistroFrame(data, registroDiario);
            janelaEditar.setVisible(true);
            this.setEnabled(false);
         }
      }
   ...
   private void doRemover() {
      if (tabelaRegistros.getSelectedColumnCount() == 1 || tabelaRegistros.getSelectedRowCount() > 0) {
   ...
   private void doExtrairXLS() {
      boolean ok = folhaPontoMensalViewState.extrairParaXLS();
      if (ok) {
   ...
   private void botaoAnoAnteriorActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botaoAnoAnteriorActionPerformed
      folhaPontoMensalViewState.anoAnterior();
      atualizarValores();
      atualizarBotoes();
   ...
   private void botaoMesAnteriorActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botaoMesAnteriorActionPerformed
      folhaPontoMensalViewState.mesAnterior();
      atualizarValores();
      atualizarBotoes();
   ...
   private void botaoProximoMesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botaoProximoMesActionPerformed
      folhaPontoMensalViewState.proximoMes();
      atualizarValores();
      atualizarBotoes();
   ...
   private void botaoProximoAnoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botaoProximoAnoActionPerformed
      folhaPontoMensalViewState.proximoAno();
      atualizarValores();
      atualizarBotoes();
