 ...
 * @author Leandro
 */
public class ConfiguracaoUsuarioController implements IConfiguracaoUsuarioController {
	...
	@Override
	public ConfiguracoesUsuarioJSON recuperarConfiguracaoUsuario(final String login) {
		final File arquivo = new File(String.format("%s/%s", PathsManager.getInstance().getPathUsuario(login), String.format(ARQUIVO_CONFIGURACAO_PATTERN, login)));
		if (arquivo.exists()) {
			try (BufferedReader reader = new BufferedReader(new FileReader(arquivo));) {
				final String conteudo = reader.readLine();
	...
	@Override
	public void sincronizar(final ConfiguracoesUsuario model) {
		if (model == null) {
			LOG.warning("ConfiguracoesUsuario null, nenhuma configuração de usuário foi sincronizada.");
			return;
		}
		final File arquivo = new File(String.format("%s/%s", PathsManager.getInstance().getPathUsuario(model.getLogin()), String.format(ARQUIVO_CONFIGURACAO_PATTERN, model.getLogin())));
		if (arquivo.exists()) {
