...
public class ArquivoController implements IArquivoController {
   ...
   public static ArquivoController getInstance() {
      if (instance == null) {
         LOG.info(String.format("Criando nova instância singleton da classe \"%s\"", ArquivoController.class.getName()));
   ...
   public String getYearPath(int ano) {
      String path = String.format("%s/%s_%d", SessaoManager.getInstance().getUsuarioAutenticado().getPathUsuario(), PREFIXO_PASTAS, ano);
      LOG.info(String.format("Recuperando diretório do ANO %d: %s", ano, path));
      return path;
   }
   ...
   public List<Integer> getAvalableYearFolders() {
      ...
      if (dir.exists()) {
         for (String path : dir.list()) {
            File file = new File(String.format("%s/%s", pathUsuario, path));
            if (file.isDirectory()) {
               ...
               if (dirSplit.length == 2 && PREFIXO_PASTAS.equalsIgnoreCase(dirSplit[0])) {
                  ...
                  try {
                     ...
                     LOG.log(Level.SEVERE, String.format("Erro ao executar parse da pasta \"%s\", o padrão \"%s\" não é um ano válido.", nomeDir, anoStr), ex);
                  }
               }
         ...
         for (File file : listFiles) {
            ...
            if (file.isFile() && EXTENSAO_ARQUIVOS.equalsIgnoreCase(ext) && PREFIXO_ARQUIVOS.equalsIgnoreCase(nomeArquivoSplit[0])) {
               ...
               try {
                  ...
                  LOG.log(Level.SEVERE, String.format("Erro ao executar parse de data da entrada \"%s\" através do padrão \"MMM\" o arquivo será descartado.", mesStr), ex);
               }
            }
         }
      }
   ...
   public String getMonthFile(int month) {
      ...
      return String.format("%s%s%s.%s", PREFIXO_ARQUIVOS, ARQUIVO_SPLITTER, DateFormatUtils.format(calendar.getTime(), SUFIXO_ARQUIVOS_PARSER), EXTENSAO_ARQUIVOS);
   }
   ...
   public File recuperarArquivo(String path, String nome, boolean criar) {
      ...
      String diretorioArquivo = String.format("%s/%s", path, nome);
      LOG.info(String.format("Recuperando arquivo: %s", diretorioArquivo));
      File arquivo = new File(diretorioArquivo);
      if (arquivo.exists()) {
         LOG.info(String.format("Arquivo %s encontrado.", nome));
      } else {
         if (criar) {
            LOG.info(String.format("Arquivo %s não foi encontrado e será criado.", nome));
            try {
               arquivo.createNewFile();
            } catch (IOException ex) {
               LOG.log(Level.SEVERE, String.format("Erro ao criar o arquivo \"%s\"", nome), ex);
            }
         } else {
            LOG.info(String.format("Arquivo %s não foi encontrado.", nome));
         }
      }
      return arquivo;
   }
